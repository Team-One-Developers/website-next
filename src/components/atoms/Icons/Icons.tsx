...
export const Icon = ({ name, color, hoverColorCSS, height, width }: Icon) => {
  const svg = () => {
    switch (name) {
      case 'Instagram':
        return {
  ...
  return (
    <svg
      ...
      width={width ? width : chosenSVG.width}
      height={height ? height : chosenSVG.height}
      fill={color ? color : chosenSVG.fill}
      className={hoverColorCSS}
    >
      {chosenSVG.svg}
    </svg>
  )
