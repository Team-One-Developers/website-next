...
const components: PortableTextComponents = {
    ...
    block: {
        ...
        h3: (() => {
            const H3 = (props: any) => (
                <h3
                    className="font-gteradisplay text-h2 mt-10 mb-6 text-black"
                    id={slugify(props.value.children[0].text)}
                >
                    {props.children}
                </h3>
        ...
        h5: (() => {
            const H5 = (props: any) => (
                <h5
                    className="font-gteradisplay text-h4 mt-8 mb-4 text-black"
                    id={slugify(props.value.children[0].text)}
                >
                    {props.children}
                </h5>
        ...
        h6: (() => {
            const H6 = (props: any) => (
                <h6
                    className="font-gteratext text-small mt-8 mb-4 font-bold text-black"
                    id={slugify(props.value.children[0].text)}
                >
                    {props.children}
                </h6>
        ...
        normal: (() => {
            const P = (props: any) => (
                <p className="font-gteratext text-medium my-4 text-black opacity-60">{props.children}</p>
            )
            return memo(P)
        })()
    },
    list: {
        bullet: ({ children }: any) => (
            <ul className="text-medium my-4 list-inside list-disc text-black opacity-60">{children}</ul>
        ),
        number: ({ children }: any) => (
    ...
    listItem: {
        bullet: ({ children }: any) => <li className="font-gteratext text-medium my-2 pl-4 text-black">{children}</li>,
...
export const T1PortableTextNew = (props: PortableTextProps) => {
