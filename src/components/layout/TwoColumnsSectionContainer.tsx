...
export const TwoColumnsSectionContainer = (
  props: TwoColumnsSectionContainerProps
) => {
  const {
    left,
    right,
  ...
  return (
    ...
    >
      {left && (
        <div
          className={twJoin('grid h-full w-full pb-8 pt-8', leftXLWidthClass)}
        >
          {left}
        </div>
      )}
      ...
      {right && (
        <div
          className={twJoin('grid h-full w-full pb-8 pt-8', rightXLWidthClass)}
        >
          <div>{right}</div>
        </div>
      )}
